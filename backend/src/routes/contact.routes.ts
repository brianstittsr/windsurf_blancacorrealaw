...
router.post('/', contactValidation, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const submissionId = await contactModel.create({
      name, email, phone, language, subject, message, status: 'new',
    });

    sendContactNotification({ name, email, phone, language, subject, message })
